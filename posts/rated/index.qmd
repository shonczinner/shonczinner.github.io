...
description: "Approval, Score, STAR, Highest-Median and BTR-Score: rated ballots end the center-squeeze honestly, but strategic manipulation still bites — STAR can be strategized and BTR-Score develops the same bad cycles Kloeckner documented"
author: "Shon Czinner"
date: "2026-09-05"
...
The strategy dynamics are similar to previous posts: there are multiple iterations where voters act strategically. For each rated method the burying takes its own form:
...
- **STAR**: give your favourite the max score, your second favourite max − 1, and bury your least favourite at 0 — steering which two score-leaders advance to the runoff.
- **Highest-Median**: same as Score (compress toward the extremes to raise your favourites' medians and collapse the bury target's).
- **BTR-Score**: bury the less-preferred of the two front-runners — rate it *below* the current last-place candidate — while maxing the preferred one, so the bury target loses any bottom-two pairwise and its score total drops.
...
Approval is the simplest rated ballot: you approve the candidates you accept, and the most-approved candidate wins. The distance-threshold variant (`d = 0.3`) is the direct rated cousin of the "vote for everyone near you" instinct. The top-two variant forces exactly two approvals — but it is worth flagging that this is not a very realistic assumption about voters. Real approval ballots don't impose a quota; voters approve everyone they accept, or everyone they'd be happy to see win. "Approve exactly the two candidates I like best" is a modelling contrivance, and, as the map below shows, the honest variant pays the price for it.

The strategic approval dynamic (adopt the preferred front-runner, truncate below) converges everywhere, and which candidate it prefers to elect is startlingly different between the two variants:
...
With honest voting, the distance-threshold variant does what the previous post's ranked methods could not: A wins everywhere. The top-two variant does not. When C sits between B and A, nearly every voter approves C (their nearest or second-nearest), so C coasts to near-universal approval and can out-poll the true median candidate. At B = −0.3 it abandons A in about half the C placements we sweep. That failure belongs to the contrived top-two assumption, not to approval voting itself.
...
Score voting elects the candidate who has the highest sum of scores on all ballots., as if each candidate's score were the strength of your
...
STAR (Score Then Automatic Runoff) first totals the scores, then holds a pairwise runoff between the top-two score-getters, with each ballot contributing its full scores to whoever it grades higher.

With honest voting, STAR elects A essentially everywhere.
...
Where the dynamics *do* converge, BTR-Score elects A — Condorcet-consistency does its job on fixed profiles. But convergence is not guarenteed under strategic voting even when a Condorcet winner does exist under honest voting.
