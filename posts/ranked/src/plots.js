...
${[...stratByB.entries()].map(([B, sgs]) => `<div id="strat-B${B}" class="chart"></div>`).join("\n")}</section>`);
  ...
  const logBlocks = report.strat.map(sg => {
    ...
    const logTxt = sg.sample ? sg.sample.log.map(e => {
      let s = `iter ${e.iter}: ${NAMES[e.winner]} → ${NAMES[e.winnerAfter]}  (${e.changedPositions}/${e.nPositions} positions changed, ${(e.changedMass * 100).toFixed(1)}% mass)`;
      if (e.trace) {
        const rounds = e.trace.rounds.map(rd => {
          const t = rd.totals.map((v, c) => `${NAMES[c]}=${v.toFixed(0)}`).join(",");
          return `[${rd.remaining.map(c => NAMES[c]).join("")}] 1st{${t}} elim ${NAMES[rd.elim]} (${NAMES[rd.low]}>${NAMES[rd.high]}=${rd.marginLowHigh.toFixed(0)}, ${NAMES[rd.high]}>${NAMES[rd.low]}=${rd.marginHighLow.toFixed(0)})`;
        }).join("; ");
        s += `\n      BTR runoff: ${rounds} => ${NAMES[e.trace.winner]}`;
      }
      if (e.ballotDist) {
        const entries = Object.entries(e.ballotDist).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k}=${v.toFixed(0)}`).join(" ");
        s += `\n      ballots: ${entries}`;
      }
      return s;
    }).join("\n") : "";
